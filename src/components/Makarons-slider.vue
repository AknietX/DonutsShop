...
const fetchProducts = async () => {
  try {
    const { data, error } = await supabase
      ...
      .eq('category', 'makarons') // Фильтруем сразу на сервере
...
const fetchSliderImages = async () => {
  try {
    ...
    if (!productsData.length) {
      console.log('Нет продуктов в категории "makarons"');
      return;
    }
  ...
  <div class="image-slider">
    <template v-if="sliderImages.length > 0">
      <img
        ...
        @error="nextImage()"
      />
      <!-- <div class="flex justify-between px-3 h-[5%] w-full items-center rounded-b-lg " style="background: #FFFBFB">
        <div class="w-[50%]">{{ products[currentIndex]?.title || 'Название продукта' }}</div>
        <div class="w-[25%] h-[100%] flex justify-between items-center ">
          <div>цена:</div>
